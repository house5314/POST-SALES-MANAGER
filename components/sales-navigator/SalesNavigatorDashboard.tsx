...
import {
  ...
  isDemoModeDisabledByEnv,
  isSalesNavigatorDemoMode,
} from "@/lib/sales/demo-sales-navigator-data";
import { fetchSigunguOptionsForSido } from "@/lib/sales/fetch-sigungu-options";
...
export const SalesNavigatorDashboard = () => {
  ...
  const [sigunguOptionsLoading, setSigunguOptionsLoading] = useState(false);
  const [sigunguSource, setSigunguSource] = useState<"api" | "static" | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    setSigunguOptions(getStaticSigunguOptions("서울특별시"));
    setSigunguSource("static");
  ...
  /** 시/도 변경 시 하위 선택과 옵션을 즉시 초기화하고 시·군·구를 API(폴백: 정적)로 조회합니다. */
  const handleSidoChange = useCallback(
    (newSido: string) => {
      ...
      if (!newSido) {
        setSigunguOptions([]);
        setSigunguSource(null);
        setSigunguStaticAsOf(undefined);
        clearPublicApiMemoryCache();
        return;
      }
      if (isDemo) {
        setSigunguOptions(getStaticSigunguOptions(newSido));
        setSigunguSource("static");
        setSigunguStaticAsOf(undefined);
        return;
      }
      setSigunguOptionsLoading(true);
      void fetchSigunguOptionsForSido(newSido).then(
        ({ options, source, staticAsOf }) => {
          setSigunguOptions(options);
          setSigunguSource(source);
          setSigunguStaticAsOf(staticAsOf);
          setSigunguOptionsLoading(false);
        }
      );
    },
    [isDemo]
  ...
  const handleFetchCommercialStores = useCallback(async () => {
    ...
    setApiNotice(
      result.quotaExceeded
        ? (result.message ?? null)
        : result.fromCache && result.message
          ? result.message
          : result.data?.notice ?? null
    );
  ...
  return (
    <div className="flex h-dvh max-h-dvh min-h-0 flex-col overflow-hidden bg-background font-sans text-sm">
      ...
      {isDemo ? (
        ...
        <div
          role="status"
          className="shrink-0 border-b border-border bg-muted/60 px-4 py-2 text-center text-xs text-muted-foreground lg:px-8"
        >
          이 배포에서는 데모 모드가 비활성화되어 있습니다(
          <span className="font-mono">NEXT_PUBLIC_DEMO_DISABLED</span>).
        </div>
      ) : null}
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden p-4 lg:flex-row lg:gap-5 lg:p-6">
          ...
          <div className="hidden min-h-0 lg:block lg:w-[20%]">
            <FilterSidebar
              ...
              sigunguOptionsLoading={sigunguOptionsLoading}
              sigunguSource={sigunguSource}
              sigunguStaticAsOf={sigunguStaticAsOf}

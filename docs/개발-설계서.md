...
| 맞춤 상담·AI 보조 | §6.3, §9 | `/api/ai/assist`, 마스킹·할루시네이션 억제 |
...
- **문제:** 생활정보홍보우편 B2B 영업에서 업체 발굴·타겟팅(배달 세대수)·제안서 작성에 시간이 많이 들고, **개인 역량·감**에 의존함.
- **해결:** 데이터 기반 상권 분석·자동 견적·ROI 가정이 포함된 제안 자료를 즉시 제공해 **업무 부담 감소·제안 신뢰도·우편 매출**에 기여.
...
| 위원 평가 (40) | 목표 부합·확산·실현가능·창의성 — 배포형 웹·문서화 완비 |
...
| GET | `/api/commercial/getEvlInfo` | `businessId`, `name` | 없음(목업) | `{ ok, info }` |
...
| 항목 | 구현 요지 |
|------|-----------|
| 입력 마스킹 | 이메일·전화·주민번호형·긴 숫자열 → 토큰 치환 후 모델 전송 |

...
- **심사·설문** 시: 배포 URL 또는 `?demo=1` 목업 모드로 API 키 없이 체험 가능
...
|------|------------------------------|-------------------------------------|
| **역할** | 상담 초안·제안서 PDF·견적 문구 | **AI 영업 포인트 요약**(상권 자동 주입) + 추가 질문 보강 |
...
| 항목 | 구현 |
|------|------|
| **실적 KPI** | 대시보드 `PocImpactPanel` — 이번 달 제안서 발행 건수, 세션 기준 평균 작성 시간, 견적 오류·시스템 견적·안전 모드 (`lib/operations/poc-kpi.ts`, 브라우저 localStorage) |
...
| Vercel Serverless | 서버 프로세스 캐시는 **콜드 스타트 시 소실** — PoC는 클라이언트 캐시가 주 |
| 상용(권장) | **Redis/KV** + 쿼터 모니터링·`numOfRows` 페이지 순회(§6 상가 API) |
...
| 2.2 | 확산성·효과성 — §1.11 KPI·피드백·시·군·구 API 동적 조회 |
| 2.3 | 실현가능·완성·윤리 — 쿼터 429·상가 페이지네이션·AI 원클릭 요약·모바일 인쇄·§1.7.1·§12.4 |
